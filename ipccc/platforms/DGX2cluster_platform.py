...
def main():
	...
	# End to end latency with NVSWitch is 10us.. Can be modeled by 1-level switches...
	...
	INTRA_CALBE_LAT = 5E-6# 10us second for end-2-end latency --> each NVLink 5us
	NV_SWITCH_PER_NODE = 1 ## Group 12 switch to 1 switch
...
'''
	...
	# </host>\r\n'''
		...
		# 4.2.2. NVSWITCH Router generate
		#fo.writelines("\r\n")
		for nvSwitchIdx in range(0,NV_SWITCH_PER_NODE):		
			line = '''	<router id="nvs'''  + str(nodeIdx*HOST_PER_NODE + nvSwitchIdx) + '''"/>\r\n'''
			fo.writelines(line)
	...
	nodePerL1Switch = L1_SWITCH_DOWN/(HOST_PER_NODE/2) #4 Plx switch per nodes; 3 nodes per L1 switch	
	...
	# This is only for 16 nodes. 2 NVswitches
	fo.writelines("<!--  Generate intra-links -->\r\n")

	for nodeIdx in range(0,totalNode):
		# 4.3.1. Node-NVSwitch links
		for hostIdx in range(0,HOST_PER_NODE):	
			line = ""
			src = str(get_host_Id(nodeIdx,hostIdx,HOST_PER_NODE))
			for nvSwitchIdx in range(0,NV_SWITCH_PER_NODE):
				nvSwitchId = nodeIdx*HOST_PER_NODE + nvSwitchIdx
				line = '''	<link id="ln''' + src + '''_nvs''' + str(nvSwitchId) + '''" ''' 
	...
	# 4.4 Generate route
	#4.4.1. Intra-route (via NVLINK/NVSWITCH)
	fo.writelines("<!--  Generate intra-route -->\r\n") 
	for nodeIdx in range(0,totalNode):
		for srcIdx in range(0,HOST_PER_NODE-1):
			for dstIdx in range(srcIdx+1,HOST_PER_NODE):
				src = str(get_host_Id(nodeIdx,srcIdx,HOST_PER_NODE))
				dst = str(get_host_Id(nodeIdx,dstIdx,HOST_PER_NODE))
				nvSwitchId = str(nodeIdx*HOST_PER_NODE)
				line = '''	<route src="n''' + src + '''" dst="n''' + dst + '''">
		<link_ctn id="ln'''+ src + '''_nvs''' + nvSwitchId +'''" />
		<link_ctn id="ln'''+ dst + '''_nvs''' + nvSwitchId +'''" />
	</route>\r\n'''
			fo.writelines(line) 
