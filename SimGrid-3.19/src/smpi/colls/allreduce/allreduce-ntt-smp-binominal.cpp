...
/*NguyenTT: Used code from allreduce-smp-binominal*/
namespace simgrid{
namespace smpi{
int Coll_allreduce_ntt_smp_binominal::allreduce(void *send_buf, void *recv_buf,
...
/*   int num_core=1;
  if (comm->is_uniform()){
    num_core = comm->get_intra_comm()->size();
 	...
 	int num_core=1;
	num_core = xbt_cfg_get_int("smpi/process_of_node");
	if ( num_core <= 0){
		THROWF(arg_error,0, "allreduce ntt_smp_binominal algorithm can't be used with %d processes per a group", num_core);  
	}
  ...
  while (mask < num_core) {
    if ((mask & intra_rank) == 0) {
      ...
      if (src < comm_size) {
		...
		//XBT_WARN("[NNNN] [%d] rbuf=[%s]",rank, print_buffer(recv_buf,count,alert));		
      }
    } else {
