...
int allreduce_ntt_lr_lr_pipeline_blocking_segment_size = 1024;
...
int Coll_allreduce_ntt_lr_lr_pipeline_blocking::allreduce(void *sbuf, void *rbuf, int rcount,
 ...
 */
	...
	if (segment_size < 0) { segment_size = allreduce_ntt_lr_lr_pipeline_blocking_segment_size;}
	...
	for (step = 0; step < pipelength + 1; step++) {
		...
		if ((step > 0) && (step < (pipelength + 1))) {
			/*2. reduce-scatter -inter between groups: the same local_rank nodes*/
			/**************************************************/
			//XBT_WARN("[NNNN] [%d] inter lr reduce-scatter",rank);
			//2.1. copy (partial of)recv_buf to send_buf
			//XBT_WARN("[NNNN] [%d] intra lr reduce-scatter",rank);
			send_offset = (((intra_rank) % intra_size) * intra_count + (step -1) * inter_segment_count) * extent;
			recv_offset = (((intra_rank) % intra_size) * intra_count + (step -1) * inter_segment_count) * extent;
			...
			for (i = 0; i < (inter_size - 1); i++) {
				send_offset = ((intra_rank * intra_count  + ((inter_rank - 1 - i + 2 * inter_size)%inter_size) * inter_count) + (step -1) * inter_segment_count) * extent;
				recv_offset = ((intra_rank * intra_count  + ((inter_rank - 2 - i + 2 * inter_size)%inter_size) * inter_count) + (step -1) * inter_segment_count) * extent;
		...
		if ((step > 1) && (step < (pipelength + 2))) {
			/*3. allgather - inter between root of each SMP node*/
			/**************************************************/
			//XBT_WARN("[NNNN] [%d] inter lr allgather",rank);
			for (i = 0; i < (inter_size - 1); i++) {
				send_offset = ((intra_rank * intra_count  + ((inter_rank - i + 2 * inter_size)%inter_size) * inter_count) + (step - 2) * inter_segment_count) * extent;
				recv_offset = ((intra_rank * intra_count  + ((inter_rank - 1 - i + 2 * inter_size)%inter_size) * inter_count) + (step - 2) * inter_segment_count) * extent;
				src = intra_rank + ((inter_rank + inter_size - 1)% inter_size)* intra_size;
				dst = intra_rank + ((inter_rank + 1)% inter_size)* intra_size;
				Request::sendrecv((char *) rbuf + send_offset, inter_segment_count, dtype, dst, tag + i, (char *) rbuf + recv_offset, inter_segment_count, dtype,src, tag + i, comm, &status);
				//XBT_WARN("[NNNN] [%d] rbuf=[%s]",rank, print_buffer(rbuf,rcount,alert));		   	
		...
		if (step > 2) {	
			...
			for (i = 0; i < (intra_size - 1); i++) {
				send_offset = (((intra_rank - i + 2 * intra_size) % intra_size) * intra_count + (step - 3) * intra_segment_count) * extent;
				recv_offset = (((intra_rank - 1 - i + 2 * intra_size) % intra_size) * intra_count + (step - 3) * intra_segment_count)  * extent;
				src = ((intra_rank + intra_size - 1) % intra_size) + inter_rank * intra_size;
				dst = ((intra_rank + 1) % intra_size) + inter_rank * intra_size;
				Request::sendrecv((char *) rbuf + send_offset, intra_segment_count, dtype, dst, tag + i, (char *) rbuf + recv_offset, intra_segment_count, dtype,src, tag + i, comm, &status);
				//XBT_WARN("[NNNN] [%d] rbuf=[%s]",rank, print_buffer(rbuf,rcount,alert));		   	
